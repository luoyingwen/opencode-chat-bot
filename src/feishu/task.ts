...
const feishuTaskStates = new Map<string, FeishuTaskState>();
...
function clearUserTaskState(userId: string, reason: string): void {
  if (feishuTaskStates.has(userId)) {
    feishuTaskStates.delete(userId);
    logger.debug(`[Feishu Task] Cleared state for user ${userId}: ${reason}`);
  }
}
...
export async function handleTaskTextInput(userId: string, text: string): Promise<string | null> {
  const state = feishuTaskStates.get(userId);
  if (!state) {
    return null;
  }
  ...
  if (state.stage === "awaiting_schedule") {
    ...
    try {
      ...
      logger.warn(`[Feishu Task] Failed to parse schedule: ${errorMessage}`);
  ...
  if (state.stage === "awaiting_prompt") {
    ...
    try {
      ...
      logger.error("[Feishu Task] Failed to save scheduled task", error);
