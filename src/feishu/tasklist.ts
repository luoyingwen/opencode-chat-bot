...
const feishuTaskListStates = new Map<string, TaskListState>();
...
function clearUserTaskListState(userId: string, reason: string): void {
  if (feishuTaskListStates.has(userId)) {
    feishuTaskListStates.delete(userId);
    logger.debug(`[Feishu TaskList] Cleared state for user ${userId}: ${reason}`);
  }
}

function getUserTaskListState(userId: string): TaskListState | null {
  const state = feishuTaskListStates.get(userId);
  if (!state) {
    return null;
  }
...
export async function handleTaskListCommand(userId: string): Promise<string> {
  try {
    ...
    feishuTaskListStates.set(userId, {
      stage: "list",
      taskId: null,
    ...
    logger.error("[Feishu TaskList] Failed to load task list", error);
  ...
  if (state.stage === "detail") {
    ...
    if (trimmedText === "删除" || trimmedText === "delete") {
      try {
        ...
        logger.error("[Feishu TaskList] Failed to delete task", error);
        return "❌ 删除任务失败";
      }
    }
