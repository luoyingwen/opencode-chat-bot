...
vi.mock("../../src/feishu/events.js", () => ({
  installFeishuEventRouting: mocked.installRoutingMock,
  setFeishuActive: mocked.setActiveMock,
}));

import { handleCommandByIndex } from "../../src/feishu/commands.js";
...
describe("feishu/commands", () => {
  ...
  it("prepares event routing before executing a command by index", async () => {
    await handleCommandByIndex("chat-1", "user-1", "3");

    expect(mocked.ensureSubscriptionMock).toHaveBeenCalledWith("Feishu", "D:/repo");
